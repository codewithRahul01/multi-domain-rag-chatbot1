...
If the legal vectorstore is missing the chain falls back to generic LLM mode.
"""
...
def _build_vectorstore() -> Optional[FAISS]:
    if not VS_PATH.exists() or not (VS_PATH / "index.faiss").exists():
        print(f"[Legal] WARNING: vectorstore not found at {VS_PATH}. Falling back to generic LLM.")
        return None
...
print("[Legal] Loading LLM and vectorstore…")
...
IMPORTANT: Always remind users that this is not a substitute for professional legal advice.
